...
import { Menu, X, Home, Volleyball, CalendarDays, Settings, User, LogOut, Calendar, Activity } from 'lucide-react';
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    ...(user ? [{ path: '/my-bookings', label: 'Lịch của tôi', icon: <CalendarDays size={15} /> }] : []),
    ...(isAdmin ? [{ path: '/admin', label: 'Quản trị', icon: <Settings size={15} /> }] : []),
  ];
  ...
  return (
    <nav className="alob-header">
        ...
        <div className="alob-header-actions">
          {user ? (
            <div className="alob-header-user">
              ...
              <button onClick={handleLogout} className="alob-header-logout" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                <LogOut size={14} />
                Đăng xuất
              </button>
            </div>
          ...
          <button
            className="md:hidden"
            style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', color: 'white', marginLeft: 4, display: 'inline-flex', alignItems: 'center' }}
      ...
      <div className="alob-nav-links hidden md:flex" style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <span className="alob-header-date" style={{ marginLeft: 'auto', fontSize: 12, color: 'rgba(255,255,255,0.8)', fontWeight: 600, alignSelf: 'center', paddingRight: 8, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
