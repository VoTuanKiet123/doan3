...
const createBooking = async (req, res) => {
  try {
    ...
    // Kiểm tra trùng lịch (checked_out cũng coi như đã kết thúc)
...
const previewFixedSchedule = async (req, res) => {
  try {
    ...
    let conflictDates = [];
    if (courtId) {
      // Kiểm tra trạng thái sân trước khi preview
      const court = await Court.findById(courtId).select("status").lean();
      if (court && court.status !== "active") {
        return res.status(400).json({
          success: false,
          message: "Sân hiện không khả dụng (đang bảo trì hoặc tạm đóng)",
        });
      }
